...
#Complete Program with last command "update" point

def add_points(device, therm_dict: dict): #put it into ddc not here
    ObjectFactory.clear_objects()
    asyncio.sleep(1)
    for class_id in therm_dict:
        # Add Analog Value
        analog_obj1 = analog_value(name=f"Mode_{class_id}", presentValue=2, properties={"units": "degreesCelsius"})
        asyncio.sleep(1)
        analog_obj2 = analog_value(name=f"Temp_set_{class_id}", presentValue=17, properties={"units": "degreesCelsius"})
        asyncio.sleep(1)
        analog_obj7 = analog_value(name=f"Temp_current_{class_id}", presentValue=17, properties={"units": "degreesCelsius"})
        asyncio.sleep(1)
        analog_obj3 = analog_value(name=f"Level_{class_id}", presentValue=1, properties={"units": "degreesCelsius"})
        asyncio.sleep(1)
        # Add Boolean Value (Binary Value)
        bool_obj = binary_value(name=f"Switch_{class_id}", presentValue=True)
        asyncio.sleep(1)
        # Add Boolean Value (Binary Value)
        bool_obj1 = binary_value(name=f"Mode_lock_{class_id}", presentValue=False)
        asyncio.sleep(1)
        # Add Boolean Value (Binary Value)
        bool_obj2 = binary_value(name=f"Temp_lock_{class_id}", presentValue=False)
        asyncio.sleep(1)
        # Add Boolean Value (Binary Value)
        bool_obj3 = binary_value(name=f"Keyboard_lock_{class_id}", presentValue=False)
        asyncio.sleep(1)
        
        # Add all objects to the application
        analog_obj1.add_objects_to_application(device)
        analog_obj2.add_objects_to_application(device)
        analog_obj3.add_objects_to_application(device)
        analog_obj7.add_objects_to_application(device)
        bool_obj.add_objects_to_application(device)
        bool_obj1.add_objects_to_application(device)
        bool_obj2.add_objects_to_application(device)
        bool_obj3.add_objects_to_application(device)
...
async def main_program():
    #Target Thermostat
    thermostat_list = [
        Thermostat(ip="192.168.12.153", device_id=1, class_id=1),
        Thermostat(ip="192.168.12.82", device_id=1, class_id=2)
    ]
    number_ther = 2
    ...
    # 6. Connect to Insight & DDC
    async with BAC0.start(ip="192.168.1.2/16", deviceId=102, port=47809,
                    localObjName="App1") as device_app:  # change ip to local device
        
        thermostat_class_dict = {t.class_id: t for t in thermostat_list}
        add_points(device_app, thermostat_class_dict)
        test_device = await BAC0.device(
            "192.168.1.1:47808", 7100, device_app, poll=10
        )#DDC IP & Device ID
        
        """ V6 BACnet Command Getting """
        ###Initial Cmd to thermostat
        for class_id in thermostat_class_dict:
            ...
            for key, prop in attrs2:
                await device_app[key].write_property('presentValue', prop)
                #await device_app["Mode1"].write_property('presentValue', current_mode)
                print(f"get {key}: ", prop)
                await asyncio.sleep(1)
        # Main Loop
        print("main loop")
        while True:
            for class_id in thermostat_class_dict:
                # 9. Check if needed to send cmd 
                print(f"=========== for Commnad of thermostat Class ID: {class_id} ==============")
                
                val = await test_device[f"Update_{class_id}"].read_property("presentValue")
                
                boolean_mapping_back = {"inactive": False, "active": True}
                update_val = boolean_mapping_back.get(str(val), True) 
                
                # 10. Only send cmd is needed
                if update_val:
                    # 7. Get Command From DDC
                    print(f"+++ Need to send Command to thermostat Class ID: {class_id} +++")
                    #1) Read all cmd values into a dict
                    attrs = [
                        ("switch",    f"Switch_{class_id}"),
                        ("mode",      f"Mode_{class_id}"),
                    ...
                    print("!!!!!!! Updating the update value !!!!!!!!")
                    
                    #await test_device[f"Update_{class_id}"].write_property('presentValue', "inactive")
                    test_device[f"Changed_{class_id}"] = True
                    val = await test_device[f"Changed_{class_id}"].read_property("presentValue")
                    print("Update value after update",val, "Type:", type(val))
                    ...
                    print(f"++++ no point value change for thermostat{class_id} +++")
